...
const navigationMenuTriggerStyle = cva(
  "group inline-flex h-9 w-max items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors \
   ...
   focus:text-green-700 focus:bg-green-50 focus:outline-none \
   disabled:pointer-events-none disabled:opacity-50 \
   data-[state=open]:text-green-700 data-[state=open]:bg-green-100"
)
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Content
    ref={ref}
    className={cn(
      "left-0 top-0 w-full rounded-md bg-white text-green-700 \
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Link
    ref={ref}
    className={cn(
      "block rounded-md px-4 py-2 text-sm font-medium text-green-600 \
    ...
    <NavigationMenuPrimitive.Viewport
      ...
      className={cn(
        "relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] \
         w-full overflow-hidden rounded-md border border-green-200 \
         bg-white shadow \
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Indicator
    ...
    {...props}
  >
    <div className="relative top-[60%] h-2 w-2 rotate-45 rounded-tl-sm bg-green-600 shadow-md" />
  </NavigationMenuPrimitive.Indicator>
))
